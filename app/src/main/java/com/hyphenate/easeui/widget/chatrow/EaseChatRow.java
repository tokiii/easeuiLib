...
//            EaseUserUtils.setUserAvatar(context, EMClient.getInstance().getCurrentUser(), userAvatarView);
            String string = context.getSharedPreferences("userInfo", 0).getString("imageUrl", "");
            string = string.replace("\\", "/");
            Glide.with(context).load(string).placeholder(R.drawable.ease_default_expression).error(R.drawable.ease_default_expression).into(userAvatarView);
//            EaseUserUtils.setUserNick(EaseUserUtils.getUserInfo(EMClient.getInstance().getCurrentUser()).getNickname(), usernickView);
            if (usernickView != null){
                usernickView.setText(context.getSharedPreferences("userInfo", 0).getString("account", EMClient.getInstance().getCurrentUser()));
            }
        } else {
